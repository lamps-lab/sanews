...
file_name = "./gpt-3.5results/guided.json"
...
with open("gpt-3.5results/guided.json") as file:
    data = json.load(file)
for item in data:
    for key in item:
        ids.append(int(key))
...
for index, sample in enumerate(shuffled_data):
	...
	messages = [
		...
		{'role':'user', 'content':
	    f"""
Given a scientific paper abstract, the task is to determine which news article was more likely written by a human; A or B. Answer with a single letter and the reasoning behind your answer.
