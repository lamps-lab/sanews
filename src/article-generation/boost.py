...
for row in abstracts:
    ...
    #print(f"prev article before loop: {previous_article}")
    ...
    for i in range(number_of_summaries-1):
        ...
        messages =  [  
            {'role':'system', 'content':'You are a Journalist who can read/understand scientific papers and writing a news article based on a scientific article.'},    
            {'role':'user', 'content':f"Read the scientific abstract and the previous news article. Draft another, improved and better version than the previous one at most {len(scialert_text.split())} words as if a human wrote it. The scientific abstract: {article}. Previous News Article: {previous_article}"}, 
        ]
        previous_article = get_completion_from_messages(messages, temperature=temperature)
        #print(f"previous_article inside loop: {previous_article}")
        news_articles.append(previous_article)
    ...
    data[id]['summary_paragraph'] = news_articles[-1]
